...
__license__   = 'GPL v3'
__copyright__ = '2014, Roman Cupisz <roman.cupisz+calibre@gmail.com>, 2015-2021 improvements by Becky <becky@fr.pl>'
...
from calibre_plugins.audible.common_utils import ReadOnlyTableWidgetItem
...
plugin_prefs = JSONConfig('plugins/Audible')
...
class GenreTagMappingsTableWidget(QTableWidget):
    ...
    def populate_table(self, tag_mappings):
        ...
        header_labels = ['Audible Genre', 'Maps to Calibre Tag(s)']
...
#         c = plugin_prefs[STORE_NAME]
        ...
        genre_group_box = QGroupBox('Audible genre to calibre tag mappings', self)
        ...
        rename_genre_button.setToolTip('Rename Audible genre')
    ...
    def add_mapping(self):
        new_genre_name, ok = QInputDialog.getText(self, 'Add new mapping',
                    'Enter a Audible genre name to create a mapping for:', text='')
        if not ok:
            # Operation cancelled
            return
    ...
    def rename_genre(self):
        ...
        if not selected_genre:
            return
        new_genre_name, ok = QInputDialog.getText(self, 'Add new mapping',
                    'Enter a Audible genre name to create a mapping for:', text=selected_genre)
        if not ok:
            # Operation cancelled
            return
        new_genre_name = unicode(new_genre_name).strip()
